...
/** Rely on the dimer method to estimate the lowest eigenvalue and its correspong. In this version a 'fictive' dimer is used, only one dimer configuration is calculated explicitly whereas the other configuration is estimated with forward differencing relative to the center of the dimer*/
...
public:
    ...
    void startNewSearchAndCompute(Matter const *matter);///< Is computational heavy! The \a matter is a pointer. The object pointed at is copied into a local copy defining the center of the dimer. 
...
//    bool determineLowestEigenmode(long maxRotations);///< Is computational heavy! Try to obtain a converged result for the eigenmode within the number of rotations passed in \a maxRotations.
    ...
    /** The rotational plane that is going to be used is determined in a Conjugated Gradient like manner.
    ...
    double calcRotationalForce(double *forceDiffOrthognalToDimer);///< Determine the rotational force on the dimers center. \forceDiffOrthognalToDimer should be the difference in forces for the 'two' configurations defining the dimer where the forces along the dimer directionNorm_ has been projected out.
};
#endif
