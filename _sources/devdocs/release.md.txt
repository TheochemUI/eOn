...
| `docs/source/releases/vX.Y.Z/` | Curated Sphinx release notes |
| Git tag `vX.Y.Z` | Annotated (prefer signed) tag; drives `release.yml` |
| GH asset `eon-vX.Y.Z.tar.xz` | `git archive` tarball (hash-stable) |
| PyPI project `eon-akmc` | Wheel/sdist via meson-python. PyPI `eon` is EoN epidemics. Import package stays `eon/`. Config: `ci/gha/pypi.ncl` |
| conda-forge `eon` | Feedstock consumes GH tarball URL + sha256 |

Towncrier writes the `## [X.Y.Z]` block from fragments under
`docs/newsfragments/`.
...
| Nickel | Generated (commit the Nickel source and the YAML together) |
|--------|------------------------------------------------------|
...
| GPG/signing keys on maintainer machines | Prefer `git tag -s` / signed commits. GHA signing is a separate channel. |

**PyPI first-time (`eon-akmc`; PyPI `eon` is EoN epidemics):**
...
win-64 includes **in-tree Fortran** (EAM/GAGAFE-class pots). See
...
- Runtime pot **DLLs** (`LoadLibrary`) sit beside in-tree Fortran in the
  conda package.
...
3. **GitHub Actions** (`release.yml`) on the tag:
   ...
   - **pypi** (stable tags `X.Y.Z` only): build + publish via trusted
   ...
   The feedstock URL requires this exact asset name.
...
again. Treat that as **incomplete multi-channel state**.
