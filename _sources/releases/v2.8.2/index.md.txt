---
myst:
  html_meta:
    "description": "Overview of the eOn v2.8.2 release, introducing uncertainty quantification for Metatomic potentials and updating core dependencies."
    "keywords": "eOn v2.8.2, release, uncertainty"
---

## [v2.8.2] - 2025-12-01

This release adds per-atom energy uncertainty quantification and variance
estimation to the [Metatomic](https://docs.metatensor.org/metatomic/latest/index.html)
interface.

The build system has been refined with new `pixi` helpers for
release management, and the **PyTorch** dependency has been updated to v2.8 to
keep compatibility with current machine learning potential stacks.

```{toctree}
:maxdepth: 2
:caption: Contents

release-notes
```

For related publications see
[2.8.0](https://eondocs.org/releases/v2.8.0/publications) and
[2.8.1](https://eondocs.org/releases/v2.8.1/publications).
