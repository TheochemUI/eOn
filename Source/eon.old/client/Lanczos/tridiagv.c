...
/* Compute eigenvalues/eigenvectors of real tridiagonal matrix using
   QR iteration with
   implicit shifts.
...
gsl_eigen_tridiagv_workspace * 
gsl_eigen_tridiagv_alloc (const size_t n)
{
  gsl_eigen_tridiagv_workspace * w ;
  ...
  w= ((gsl_eigen_tridiagv_workspace *) malloc (sizeof(gsl_eigen_tridiagv_workspace)));
...
void
gsl_eigen_tridiagv_free (gsl_eigen_tridiagv_workspace * w)
{
  free(w->gs);
  free(w->gc);
  free(w->sd);
...
int
gsl_eigen_tridiagv (const gsl_vector * diag, const gsl_vector * sub_diag, gsl_vector * eval,
            gsl_matrix * evec, gsl_eigen_tridiagv_workspace * w)
{
  if (diag->size != sub_diag->size+1)
    {
      GSL_ERROR ("sub-diagonal vector must match matrix size-1", GSL_EBADLEN);
    }
  else if (eval->size != diag->size)
    {
      GSL_ERROR ("eigenvalue vector must match matrix size", GSL_EBADLEN);
    }
  else if (evec->size1 != diag->size || evec->size2 != diag->size)
    {
  ...
  else
    {
      ...
      if (N == 1)
        {
          double A00 = gsl_vector_get (diag, 0);
...

      {
        ...
        gsl_vector_memcpy(&d_vec.vector, diag);
        gsl_vector_memcpy(&sd_vec.vector, sub_diag);
