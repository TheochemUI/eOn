...
def savecon(fileout, p, w = 'w'):
    ...
    if hasattr(fileout, 'write'):
        ...
        con = open(fileout, w)
    print("Generated by eOn\n",con)
    dim = len(p.r[0])
    lengths, angles = box_to_length_angle(p.box)
    print(" ".join(['%.6f' % s for s in lengths]), con)
    print(" ".join(['%.6f' % s for s in angles]), con)
    print("\n\n", con)
    atom_count = {}
    name_order = []
    for i in range(len(p)):
        ...
        if name in atom_count:
            ...
            atom_count[name] = 1
    print(len(name_order), con)
    print(" ".join([str(atom_count[i]) for i in name_order]), con)
    printmasses = []
    index = 0
    for i in range(len(name_order)):
        printmasses.append(p.mass[index])
        index += atom_count[name_order[i]]
    print(" ".join(["%.6f"% i for i in printmasses]), con)
    index = 0
    for i in range(len(name_order)):
        print(name_order[i], con)
        print("Coordinates of component", i+1, con)
