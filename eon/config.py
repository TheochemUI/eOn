...
class ConfigClass:
    def __init__(self):
        ...
        yaml_file = Path(__file__).with_name("config.yaml")
        y = yaml.load(yaml_file.read_text(), Loader=yaml.BaseLoader)
...
#           self.main_random_seed = None
        ...
        if not gave_config and Path(self.path_root).resolve() != Path.cwd().resolve():
