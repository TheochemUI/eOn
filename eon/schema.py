...
class BasinHoppingConfig(BaseModel):
    ...
    write_unique: bool = Field(
        default=False,
        description="If true, the client will write out all the unique geometries and their energies to files named "
        "min_xxxxx.con and energy_xxxxx.con, where 'xxxxx' corresponds to the Monte Carlo step.",
    )
    ...
    push_apart_distance: float = Field(
        default=0.4,
        description="Push atoms apart until no atoms are closer than this distance. This criterion is enforced for "
        "the initial structure and all those generated by random displacement.",
    )
