...
For BLAS-bound workloads (large GPRD / Hessian / NEB), pass
`-Dwith_flexiblas=true` (meson) or `-DWITH_FLEXIBLAS=ON` (cmake)
to link Eigen via [FlexiBLAS](https://github.com/mpimd-csc/flexiblas).
One eonclient binary then forwards every BLAS / LAPACK call to any
installed backend at run time, picked by the `FLEXIBLAS` env var:

```{code-block} bash
meson setup builddir -Dwith_flexiblas=true
meson compile -C builddir
FLEXIBLAS=OPENBLAS ./eonclient   # OpenBLAS at run time
FLEXIBLAS=INTELMKL ./eonclient   # Intel MKL at run time
FLEXIBLAS=BLIS     ./eonclient
```

`subprojects/flexiblas.wrap` pins upstream v3.5.0 and CMake-builds
FlexiBLAS when no system install is found, so the option never
fails for "no FlexiBLAS available". Mutually exclusive with the
legacy `-Duse_mkl=true` direct-MKL link; FlexiBLAS can route to
MKL at run time via `FLEXIBLAS=INTELMKL` instead.
