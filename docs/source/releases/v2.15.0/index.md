...
myst:
  html_meta:
    "description": "eOn v2.15.0: runtime Fortran potentials, process-search fixed-atom fix, LAMMPS/MPI NEB isolation, release process (GH/PyPI/feedstock) automation."
...
only after pre-release checks pass; this page is authored **before** the version
chore so the cut is not blocked on writing notes.

This release adds runtime-loaded Fortran potentials (`potentials_path` / `dlopen`),
