...
- `pip_metatomic` keeps a private client process; it skips the shared `guard`
  used by the pure-Python potentials
- Models are usually `.pt` files
...
Use non-conservative forces for speed only when the model documents them.
Energy conservation in MD or long NEB runs is then model-dependent.
