...
namespace forcefields {
      ...
      public:
            ...
            ~ZhuPhilpott();
            void computeHH_O_Pt_(
                  const int nWater, const int nPt,
                  const double r[], double f[], double & energy,
                  double const b[], bool const fixed[]
            );
            void computeHH_O_(
                  const int nWater, const double r[], double f[], double & energy, double const b[], bool const fixed[]
            );
            int nPlatinum() const;
      ...
      private:
            ...
            void coulombWithCutoff(
                  Water& w1, Water& w2,
                  double& u, double const relativePermittivity);
            void coulombFull(
                  Water & w1, Water & w2,
                  double & U, double const relativePermittivity);
            int nPlatinum_;
