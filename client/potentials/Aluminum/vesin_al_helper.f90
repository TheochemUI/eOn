...
!> Half neighbour list for the EAM Aluminum potential, built with vesin.
!!
...
!! atom `i`, in ascending order. GAGAFE turns that into its own
!! `indpra`/`iptpr1` layout and derives the separation vectors itself, so the
!! minimum-image convention and the sign of `delpr` stay with the caller.
module vesin_al
  ...
  private
  public :: vesin_al_build, vnl_row, vnl_jlist, vnl_npairs
...
contains
  ...
  !!
  !! `ierr` is 0 on success, 1 when vesin fails, 2 when the list needs more
  !! than `maxpairs` entries.
  subroutine vesin_al_build(natoms, rpos, ax, ay, az, cutoff, maxpairs, ierr)
    integer, intent(in) :: natoms
    real(c_double), intent(in) :: rpos(3*natoms)
    real(c_double), intent(in) :: ax, ay, az, cutoff
    integer, intent(in) :: maxpairs
    ...
    call nl%compute(points, box, periodic=[.true., .true., .true.], &
                    status=vstatus)
    if (vstatus /= 0) then
      write(*,*) 'Aluminum: vesin neighbour build failed: ', nl%errmsg
      ierr = 1
      return
    end if
    ...
    call grow_int(vnl_jlist, max(maxpairs, 1))
