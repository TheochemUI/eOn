...
int linmin() {
  ...
  if ((cg_infolevel > 0) && (0 == myid)) {
    printf("after mnbrak alpha_a %le alpha_b %le alpha_c %le %.12lf %.12lf "
           "%.12lf\n",
  ...
  if ((cg_infolevel > 0) && (0 == myid)) {
    printf(
        "iter1= %d iter2 = %d alphamin = %e f_max = %e fnorm %e  epot %.12e \n",
...
/*       if (fb>=old_cg_poteng) */
/* 	{ */
/* 	  acg_alpha = (acg_alpha<=1e-10 || iter>50) ? acg_alpha *acg_incfac :
 * acg_alpha * acg_decfac; */
/* 	} */
/*       if (iter > 100) */
/* 	{ */
/* 	  printf("iter > 100"); */
