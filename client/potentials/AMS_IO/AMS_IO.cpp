...
void AMS_IO::force(long N, const double *R, const int *atomicNrs, double *F, double *U, const double *box, int nImages=1)
{
    passToSystem(N, R, atomicNrs, box); 
    system("chmod +x run_AMS_IO.sh");
    system("./run_AMS_IO.sh >> ams_output"); // Run a single point AMS_IO calculation and write the results into ams_output
    recieveFromSystem(N, F, U);
    return;
}


void AMS_IO::passToSystem(long N, const double *R, const int *atomicNrs, const double *box)
...
void AMS_IO::recieveFromSystem(long N, double *F, double *U)
{
    ...
    if (strcmp(line, "  Index   Atom            d/dx            d/dy            d/dz\n") == 0){ // Finding the forces
      ...
      fscanf(in, "%lf %s %lf %lf %lf", &index, &junkChar, &forceX, &forceY, &forceZ);
        F[int(i) * 3 + 0] = -forceX;
        F[int(i) * 3 + 1] = -forceY;
        F[int(i) * 3 + 2] = -forceZ; // AMS_IO gives gradients, not forces, hence the change.
      }
     }
    }
        for (int i = 0; i < 3*N ; i++) {
