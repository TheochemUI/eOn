...
std::string ensure_extension(const std::string &filename,
                             std::string_view ext) {
...
void matter2xyz(Matter &m, std::string filename, bool append) {
  ...
  // (void)-cast every fprintf return because we have no recovery
  // path mid-write -- caller-side error reporting is on the
  // !readable() branch above.
  (void)std::fprintf(file, "%ld\nGenerated by eOn\n", m.numberOfAtoms());
  ...
  for (i = 0; i < m.numberOfAtoms(); i++) {
    (void)std::fprintf(file, "%s\t%11.6f\t%11.6f\t%11.6f\n",
                       atomicNumber2symbol(m.getAtomicNr(i)),
                       m.getPosition(i, 0), m.getPosition(i, 1),
                       m.getPosition(i, 2));
  }
  (void)std::fclose(file);
}

void writeTibble(Matter &m, const std::string &fname) {
