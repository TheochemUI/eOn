...
int Prefactor::getPrefactors(Parameters *parameters, Matter *min1,
  ...
  if (parameters->prefactor.filterScheme == Prefactor::FILTER_FRACTION) {
  ...
  // remove zero modes
  if (parameters->structcomp.checkRotation) {
  ...
  if (parameters->prefactor.rate == Prefactor::RATE_HTST) {
  ...
  } else if (parameters->prefactor.rate == Prefactor::RATE_QQHTST) {
    float kB_T = parameters->main.temperature * 8.617332e-5; // eV
...
VectorXi Prefactor::movedAtoms(Parameters *parameters, Matter *min1,
  ...
  for (int i = 0; i < nAtoms; i++) {
    if ((diffMin1.row(i).norm() > parameters->prefactor.minDisplacement) ||
        (diffMin2.row(i).norm() > parameters->prefactor.minDisplacement)) {
      ...
      for (int j = 0; j < nAtoms; j++) {
        ...
        if (diffRSaddle < parameters->prefactor.withinRadius &&
            (!saddle->getFixed(j))) {
...
VectorXi Prefactor::movedAtomsPct(Parameters *parameters, Matter *min1,
  ...
  SPDLOG_DEBUG(
      ...
      parameters->prefactor.filterFraction * 100);
  double sum = 0.0;
  int mini = 0;
  for (int i = 0; i < nAtoms; i++) {
  ...
  int nMoved = 0;
  double d = 0.0;
  while (d / sum <= parameters->prefactor.filterFraction && nMoved < nFree) {
    int maxi = mini;
    for (int i = 0; i < nAtoms; i++) {
      if (diff[i] >= diff[maxi]) {
        if (!(moved.array() == i).any()) {
