...
int ConjugateGradients::line_search(double maxMove)
{
    ...
    // move system an infinitesimal step to determine the optimal step size
...
int ConjugateGradients::single_step(double maxMove)
{
    ...
    if (!parameters->optCGNoOvershooting)
    {
        ...
        // negative if product of the projected forces before and after
        // the step are in opposite directions
