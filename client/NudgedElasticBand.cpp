...
NudgedElasticBand::NudgedElasticBand(Matter *initialPassed,
                                     Matter *finalPassed,
...
int NudgedElasticBand::compute(void) {
    ...
    while (!objf.isConverged()) {
        ...
        double dE = image[maxEnergyImage]->getPotentialEnergy() - image[0]->getPotentialEnergy();
        double stepSize
...
void NudgedElasticBand::updateForces(void) {
    ...
    for (long i = 1; i <= images; i++) {
        ...
        if (parameters->nebDoublyNudged) {
            forceSpringPerp
                = forceSpring - (forceSpring.array() * (*tangent[i]).array()).sum() * *tangent[i];
        ...
        if (parameters->nebClimbingImageMethod && i == maxEnergyImage) {
            ...
            *projectedForce[i]
                = force - (2.0 * (force.array() * (*tangent[i]).array()).sum() * *tangent[i])
                  + forceDNEB;
...
void NudgedElasticBand::printImageData(bool writeToFile) {
    ...
    AtomMatrix tangentEnd
        = image[images]->pbc(image[images + 1]->getPositions() - image[images]->getPositions());
    AtomMatrix tang;
...
int NEBObjectiveFunction::degreesOfFreedom() { return 3 * neb->images * neb->atoms; }
