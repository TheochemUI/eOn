        ...
        void setPositions(VectorXd x)
        {
            ...
            for (long i = 1; i <= neb->images; i++) {
                neb->image[i]->setPositions(MatrixXd::Map(
                    x.segment(3*neb->atoms * (i-1), 3*neb->atoms).data(),
                    neb->atoms, 3));
            }
        }
...
int NudgedElasticBand::compute(void)
{
    ...
    while (!objf.isConverged()) {
        ...
        double dE = image[maxEnergyImage]->getPotentialEnergy() - 
                    image[0]->getPotentialEnergy();
        double stepSize =
            helper_functions::maxAtomMotionV(image[0]->pbcV(objf.getPositions()-pos));
        if (dE > 0.01)
...
void NudgedElasticBand::updateForces(void)
{
    ...
    for (long i = 1; i <= images; i++) {
        ...
        forcePerp = force -
            (force.array() * (*tangent[i]).array()).sum() * *tangent[i];

        // calculate the spring force
        distPrev = sqrt(image[i]->pbc(posPrev - pos).squaredNorm());
        distNext = sqrt(image[i]->pbc(posNext - pos).squaredNorm());
        ...
        if (parameters->nebDoublyNudged) {
            ...
            if (parameters->nebDoublyNudgedSwitching) {
                double switching = 2.0 / M_PI * atan(pow(forcePerp.norm(), 2)
                                   / pow(forceSpringPerp.norm(), 2)); 
                forceDNEB *= switching;
            }
        } else {
            forceDNEB.setZero();
        ...
        if(parameters->nebClimbingImageMethod && i==maxEnergyImage) {
            ...
            *projectedForce[i] = force -
                (2.0 * (force.array() * (*tangent[i]).array()).sum() *
                *tangent[i]) + forceDNEB;
...
void NudgedElasticBand::printImageData(bool writeToFile)
{
    ...
    AtomMatrix tangentStart = image[0]->pbc(
        image[1]->getPositions() - image[0]->getPositions());
    AtomMatrix tangentEnd = image[images]->pbc(
        image[images+1]->getPositions() - image[images]->getPositions());
    AtomMatrix tang;
...
void NudgedElasticBand::findExtrema(void)
{
    ...
    for (long i = 0; i <= images; i++) {
        ...
        if (i == 0) {
            tangentEndpoint =
                image[i]->pbc(image[1]->getPositions() - image[0]->getPositions());
            tangentEndpoint.normalize();
            F1 = (image[i]->getForces().array() * 
                tangentEndpoint.array()).sum() * dist;
        } else {
            F1 = (image[i]->getForces().array() *
                (*tangent[i]).array()).sum() * dist;
        }
        if (i == images) {
            tangentEndpoint = image[i+1]->pbc(image[images+1]->getPositions() -
                image[images]->getPositions());
            tangentEndpoint.normalize();
            F2 = (image[i+1]->getForces().array() *
                  tangentEndpoint.array()).sum() * dist;
        } else {
            F2 = (image[i+1]->getForces().array() *
                 (*tangent[i+1]).array()).sum() * dist;
        }
