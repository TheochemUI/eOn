...
std::vector<std::string> ParallelReplicaJob::run(void)
{
    ...
    *trajectory = *reactant;
    ...
    int stateCheckInterval = int(floor(params->parrepStateCheckInterval/params->mdTimeStep+0.5));
    int recordInterval = int(floor(params->parrepRecordInterval/params->mdTimeStep+0.5));
    ...
    for (int step=1; step<=params->mdSteps; step++) {
        ...
        double kinT = (2.0*kinE/(trajectory->numberOfFreeAtoms()*3)/params->kB);
        ...
        if (step % stateCheckInterval == 0 || step == params->mdSteps) {
            ...
            if (!min.compare(reactant) && transitionTime == 0) {
                ...
                log("%s Transition time: %.3e s\n", LOG_PREFIX, transitionTime*params->timeUnit*1e-15);
    ...
    int decorrelationSteps = int(floor(params->parrepCorrTime/params->mdTimeStep+0.5));
...
void ParallelReplicaJob::dephase(Matter *trajectory)
{
    ...
    int dephaseSteps = int(floor(params->parrepDephaseTime/params->mdTimeStep+0.5));
