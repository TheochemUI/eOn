...
{
    long i, step, samplingSteps =
        long(parameters->repexcSamplingTime / parameters->mdTimeStep + 0.5);
    long exchangePeriodSteps =
        long(parameters->repexcExchangePeriod / parameters->mdTimeStep + 0.5);
    ...
    if (parameters->repexcTemperatureDistribution == "linear") {
        for (i = 0; i < parameters->repexcReplicas; i++) {
            replicaTemperature[i] = parameters->repexcTemperatureLow +
                double(i) / double(parameters->repexcReplicas - 1.0)
        ...
        double kTemp = log(parameters->repexcTemperatureHigh /
                           parameters->repexcTemperatureLow) /
                          (parameters->repexcReplicas - 1.0);
    ...
    log("\nReplica Exchange sampling for %.0f fs; %ld steps; %ld replicas.\n",
         parameters->repexcSamplingTime * 10.18, samplingSteps,
         parameters->repexcReplicas);

    for (step = 1; step <= samplingSteps; step++) {
        for (i = 0; i < parameters->repexcReplicas; i++) {
            replicaDynamics[i]->oneStep();
            cout <<"step: " <<step <<" i " <<i <<" energy: "
                 <<replica[i]->getPotentialEnergy()<<endl;
        }
        if ((step % exchangePeriodSteps) == 0) {
            for (long trial = 0; trial < parameters->repexcExchangeTrials; trial++)
            {
                ...
                cout <<"step: " <<step <<" trial swap, i " <<i <<" elow: " <<energyLow
                     <<" ehigh: " <<energyHigh <<" pAcc: " <<pAcc <<" rand: " <<tmp;
                if (tmp<pAcc) {
                    cout <<" swap\n";
                    // swap configurations
                    tmpMatter = replica[i];
