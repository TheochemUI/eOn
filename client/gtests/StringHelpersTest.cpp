...
TEST_F(StringHelpersTest, TestIsNotNum) {
  ...
  for (auto substring : split_strings) {
    EXPECT_EQ(eonc::helpers::isNumber(substring), false);
  }
}
...
TEST_F(StringHelpersTest, TestIsNum) {
  ...
  EXPECT_EQ(split_strings.size(), 4);
  for (auto substring : split_strings) {
    EXPECT_EQ(eonc::helpers::isNumber(substring), true);
  }
  EXPECT_EQ(eonc::helpers::isNumber("2.6"s), true);
}
...
TEST_F(StringHelpersTest, TestEmpty) {
  ...
  EXPECT_DEATH(eonc::helpers::get_val_from_string<double>(number_string, 1),
               "(not line.empty())"s);
  EXPECT_DEATH(eonc::helpers::get_val_from_string<size_t>(number_string),
               "(not line.empty())"s);
}
...
TEST_F(StringHelpersTest, TestNeg) {
  ...
  EXPECT_DEATH(eonc::helpers::get_val_from_string<size_t>(tester),
               "Can't represent negative numbers with an unsigned type"s);
  // Should be OK, due to truncation
  EXPECT_EQ(eonc::helpers::get_val_from_string<size_t>(tester, 1)[0], 1);
  // Fine otherwise
  std::vector<double> exp_vals = {1.0, -1.0};
  EXPECT_EQ(eonc::helpers::get_val_from_string<double>(tester), exp_vals);
}
