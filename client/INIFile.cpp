...
bool CIniFile::SetValue(unsigned const keyID, unsigned const valueID,
                        string const value) {
  if (keyID < keys.size() && valueID < keys[keyID].names.size())
...
bool CIniFile::SetValue(string const keyname, string const valuename,
                        string const value, bool const create) {
  long keyID = FindKey(keyname);
  if (keyID == noID) {
    if (create)
...
bool CIniFile::SetValueI(string const keyname, string const valuename,
                         int const value, bool const create) {
...
bool CIniFile::SetValueF(string const keyname, string const valuename,
                         double const value, bool const create) {
  char svalue[MAX_VALUEDATA];
...
bool CIniFile::SetValueV(string const keyname, string const valuename,
                         char *format, ...) {
  va_list args;
...
string CIniFile::GetValue(unsigned const keyID, unsigned const valueID,
                          string const defValue) const {
  if (keyID < keys.size() && valueID < keys[keyID].names.size())
...
string CIniFile::GetValue(string const keyname, string const valuename,
                          bool &found) const {
  long keyID = FindKey(keyname);
  if (keyID == noID) {
    found = false;
...
int CIniFile::GetValueI(string const keyname, string const valuename,
...
long CIniFile::GetValueL(string const keyname, string const valuename,
                         long const defValue) const {
  bool found;
  string value = GetValue(keyname, valuename, found);
  if (found)
...
double CIniFile::GetValueF(string const keyname, string const valuename,
...
bool CIniFile::GetValueB(string const keyname, string const valuename,
                         bool const defValue) const {
  bool found;
  string value = GetValue(keyname, valuename, found);
  if (found)
...
unsigned CIniFile::GetValueV(string const keyname, string const valuename,
                             char *format, void *v1, void *v2, void *v3,
  ...
  nVals = sscanf(value.c_str(), format, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10,
...
string CIniFile::KeyComment(unsigned const keyID,
                            unsigned const commentID) const {
  if (keyID < keys.size() && commentID < keys[keyID].comments.size())
    return keys[keyID].comments[commentID];
  return "";
}

string CIniFile::KeyComment(string const keyname,
                            unsigned const commentID) const {
  long keyID = FindKey(keyname);
  if (keyID == noID)
    return "";
...
bool CIniFile::DeleteKeyComment(unsigned const keyID,
...
bool CIniFile::DeleteKeyComment(string const keyname,
                                unsigned const commentID) {
  long keyID = FindKey(keyname);
  if (keyID == noID)
    return false;
