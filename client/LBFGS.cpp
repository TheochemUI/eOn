...
Eigen::VectorXd LBFGS::getStep(double a_maxMove, const Eigen::VectorXd &a_f) {
  ...
  if (m_iteration > 0) {
    ...
    if (C < 0) {
      ...
      return eonc::geometry::maxAtomMotionAppliedV(1000 * f, a_maxMove);
    }
  ...
  if (distance >= a_maxMove && m_optConfig.opts.lbfgs.distance_reset) {
    QUILL_LOG_DEBUG(m_log,
                    ...
                    distance);
    reset();
    return eonc::geometry::maxAtomMotionAppliedV(H0 * f, a_maxMove);
  }
...
int LBFGS::step(double a_maxMove) {
  ...
  if (!accepted) {
    ...
    reset();
    m_objf->setPositions(
        r + eonc::geometry::maxAtomMotionAppliedV(0.1 * f, a_maxMove));
    e_acc = m_objf->getEnergy();
  }
