...
ImprovedDimer::ImprovedDimer(std::shared_ptr<Matter> matter,
  ...
  *x1 = *matter;
  ...
  if (params->dimer.optMethod == OPT_CG) {
    init_cg = true;
  }
...
void ImprovedDimer::compute(std::shared_ptr<Matter> matter,
  ...
  *x1 = *matter;
  ...
  if (params->dimer.optMethod == OPT_LBFGS) {
    s.clear();
    y.clear();
    rho.clear();
    init_lbfgs = true;
  ...
  do // while we have not reached phi_tol or maximum rotations.
  {
    ...
    statsTorque = F_R.norm() / (params->main.finiteDifference * 2.0);

    // Determine the step direction, theta
    if (params->dimer.optMethod == OPT_SD) // steepest descent
    {
      theta = F_R / F_R.norm();
    } else if (params->dimer.optMethod == OPT_CG) // conjugate gradients
    {
      if (init_cg) {
        init_cg = false;
        gamma = 0.0;
      ...
      F_R_Old = F_R;
    } else if (params->dimer.optMethod == OPT_LBFGS) // quasi-newton
    {
      if (init_lbfgs == false) {
    ...
    if (abs(phi_prime) > phi_tol) {
      ...
      *x1p = *x1;
      ...
      statsTorque = F_R.norm() / (2.0 * params->main.finiteDifference);
      statsRotations += 1;
      SPDLOG_LOGGER_INFO(
          log,
          "[IDimerRot]  -----   ---------   ----------   ------------------   "
      ...
      SPDLOG_LOGGER_INFO(
          ...
          C_tau, F_R.norm() / delta);
           ...
           statsRotations < params->dimer.rotationsMax);
}
