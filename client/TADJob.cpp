...
std::vector<std::string> TADJob::run(void)
{
    ...
    *reactant = *current;
    ...
    log("High temperature MD simulation running at %.2f K "
        "to simulate dynamics at %.2f K\n",
...
int TADJob::dynamics()
{
    ...
    while (!stopFlag)
    {
        ...
        if(transitionFlag) {
            ...
            *current = *mdBuffer[refineStep-1];
            ...
            log("tranisitonTime= %.3e s, Barrier= %.3f eV, correctedTime= %.3e s, "
                ...
                transitionTime * 1e-15 * parameters->timeUnit, barrier, 
                correctedTime * 1e-15 * parameters->timeUnit,
                sumSimulatedTime * 1e-15 * parameters->timeUnit,
                minCorrectedTime * 1.0e-15 * parameters->timeUnit,
                stopTime * 1.0e-15 * parameters->timeUnit);
        ...
        if ((step % tenthSteps == 0) || (step == parameters->mdSteps)) {
            ...
            log("progress: %3.0f%%, max displacement: %6.3lf, step %7ld/%ld\n",
                (double)100.0 * step / parameters->mdSteps, maxAtomDistance, step,
                parameters->mdSteps);
        }
    ...
    log("\nTemperature : Average = %lf ; Stddev = %lf ; Factor = %lf; "
        ...
        minCorrectedTime / step / parameters->mdTimeStep);
    if (isfinite(avgT) == 0) {
...
bool TADJob::saddleSearch(Matter *cross){
    ...
    dimerSearch = new MinModeSaddleSearch(
        cross, mode, reactant->getPotentialEnergy(), parameters);
